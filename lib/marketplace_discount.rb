class PromotionDiscount
  ...
  def discount_amount
    ...
    total_discount += (0.75*items.count('001')) if lavender_heart_promo?
  ...
  def lavender_heart_promo?
    true if items.count('001') >= 2 && rules.include?('lavender_heart_promo')
  end
...
class Checkout
  ...
  def total
    ...
    discount = PromotionDiscount.new(:rules => promotional_rules, 
                                     :items => scanned, 
